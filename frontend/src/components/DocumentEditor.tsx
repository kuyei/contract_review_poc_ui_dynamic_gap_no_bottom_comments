...
export type DocumentEditorHandle = {
  locateRisk: (opts: { targetText?: string; anchorText?: string; evidenceText?: string; clauseUids?: string[] }) => void
  scrollToBlock: (blockId: string) => void
  scrollToEdit: (editId: string) => void
  applyAiPatch: (opts: { patchId?: string | number; targetText?: string; revisedText?: string }) => boolean
...
function createReplacementFragment(
  ...
  for (const token of tokens) {
    ...
    if (sourceEl && sourceEl instanceof HTMLElement) {
      const wrapper = sourceEl.cloneNode(false) as HTMLElement
...
>(function DocumentEditor(props, ref) {
  ...
  const applyAiPatch = (opts: { patchId?: string | number; targetText?: string; revisedText?: string }) => {
    ...
    if (!targetText || !revisedText) return false
    if (targetText === revisedText) return false
    ...
    if (!matched) {
      const compactTarget = normalizeSearchText(targetText)
      for (const el of blockElsRef.current.values()) {
        const txt = plainTextOf(el)
        if (compactTarget && normalizeSearchText(txt).includes(compactTarget)) {
          matched = el
          break
        }
      }
    ...
    const exactMatches = findAllOccurrences(currentText, targetText)
    let nextText = currentText
    let startIndex = -1
    let endIndex = -1
    let keepUnderlinedDigits = false
    let effectiveTargetText = targetText
    let replaced = false

    const candidateRanges: Array<{ start: number; end: number }> = exactMatches.map((idx) => ({
      start: idx,
      end: idx + targetText.length
    }))
    if (candidateRanges.length === 0) {
      candidateRanges.push(...findCompactOccurrencesWithRawRange(currentText, targetText))
    }
    ...
    if (patchId) {
      appliedAiPatchMapRef.current.set(patchId, {
        ...
        targetText: effectiveTargetText || targetText,
        revisedText,
        startIndex,
        endIndex,
